...
describes what users get. Pick a format with `-o`, or let gutendex choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gutendex <command> -o table   # aligned columns for reading
gutendex <command> -o jsonl   # one JSON object per line, for piping
gutendex <command> -o json    # a single JSON array
gutendex <command> -o csv     # spreadsheet friendly
gutendex <command> -o tsv     # tab-separated
gutendex <command> -o url     # just the URL column
gutendex <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gutendex <command> --fields id,title,url
```
...
```bash
gutendex <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
gutendex <command>            # a table, because this is a terminal
gutendex <command> | wc -l    # JSONL, because this is a pipe
```
